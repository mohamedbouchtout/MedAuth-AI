...
def test_both_surfaces_are_mounted() -> None:
    ...
    application's list, so a path search over ``app.routes`` finds only the docs
    endpoints. ``url_path_for`` is the supported way to ask.
    """
    app = create_app()

    assert app.url_path_for("health") == "/health"
    assert app.url_path_for("nudge_stream", session_id="abc") == "/ws/nudges/abc"
...
def test_the_websocket_is_absent_from_the_openapi_document() -> None:
    ...
    ``docs/api/nudge-service.yaml`` documents it in prose for that reason.
    """
...
def test_nothing_connects_at_startup() -> None:
    ...
    Redis connects on first command and a subscription belongs to a connection
    rather than to the process, so there is nothing to open here.
    """
