...
if it touches PHI. The route tests cover the "if not" half — a refused
connection writes nothing. This covers the "if" half, and the shape of the row.
"""
...
async def test_the_row_identifies_the_provider_and_the_encounter(
    ...
    await audit.audit_audio_stream(session_id=session_id, provider_id=provider_id)
...
async def test_the_row_names_this_service(recorded: list[dict[str, Any]]) -> None:
    ...
    assert recorded[0]["service_name"] == "audio-ingestion"


async def test_the_client_address_and_agent_are_recorded_when_known(
    recorded: list[dict[str, Any]],
) -> None:
    await audit.audit_audio_stream(
        session_id=uuid.uuid4(),
        provider_id=uuid.uuid4(),
        ip_address="10.1.2.3",
...
async def test_no_transcript_or_audio_field_can_reach_the_row(
    recorded: list[dict[str, Any]],
) -> None:
