...
substitute a fake through ``app.dependency_overrides`` without a real Redis in
reach. Mirrors track-a-clinical's module of the same name; this service has no
database-backed routes yet, so it declares no session dependency — the assembly
opens its own session through the consumer's factory.
"""
