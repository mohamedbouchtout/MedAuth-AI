...
class EHRAdapter:
    ...
    **The HTTP client is injected, never constructed here.** It is the
    ...
    async def _search_with(
        ...
        """
        resources, _ = await self._search_page(resource_type, params, resource_id=resource_id)
        return resources

    async def _search_page(
        self, resource_type: str, params: dict[str, str], *, resource_id: str
    ) -> tuple[list[dict[str, Any]], int | None]:
        """Run a search and return the entry resources *and* the Bundle's ``total``.

        Every caller that only wants the resources goes through
        :meth:`_search_with`. This variant exists for :meth:`search_patients`,
        which has to tell "these are all the matches" apart from "these are the
        first page of them" — and the page it received is not enough to do that,
        because a server may cap ``_count`` below what was asked for.

        ``total`` is the number of matches across all pages. It is **optional**
        in a searchset Bundle, so ``None`` is an ordinary answer and not an
        error; a server that omits it has told us nothing about how many matches
        exist. Anything that is not a non-negative integer is read as absent
        rather than trusted — ``bool`` is excluded explicitly because it is an
        ``int`` in Python and ``True`` would otherwise be read as a total of 1.
        """
    ...
    async def search_patients(
        ...
        **An empty result is a successful answer**, on the same terms as the
        ...
        resources, total = await self._search_page(
            "Patient", params, resource_id=SEARCH_RESOURCE_ID
        )
        ...
        # Two independent signals, because neither one covers the other. The
        # over-fetch catches a server that honoured `_count`: more came back than
        # fit, so more exist. `total` catches a server that capped `_count` below
        # what was asked for — then a short page is *not* proof there is nothing
        # more, and the page alone would report `truncated=False` for a name
        # matching hundreds. Whichever says there are more is believed.
        #
        # Neither fires for a server that both caps `_count` and omits `total`,
        # which is conformant — `total` is optional in a searchset Bundle. That
        # case still under-reports, and following the Bundle's `next` link is
        # what would close it. Recorded rather than silently accepted, because a
        # provider who is not told the list is partial concludes the patient is
        # not in the system.
        truncated = len(patients) > limit or (total is not None and total > len(matches))
        return PatientSearchResults(matches=matches, truncated=truncated)
