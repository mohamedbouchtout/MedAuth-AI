...
describe('visitPhase — session states', () => {
  /**
   * Unlike the mobile twin's, this app's `ended` carries the session it closed:
   * ending a visit here navigates to the note review screen (TASK-071), which is
   * keyed on `session_id`, so the identifier has to survive the transition.
   */
  it('keeps the session on the completed phase, for the note review screen', () => {
    expect(visitPhase({ kind: 'ended', session: SESSION }, { status: 'idle' }).kind).toBe('ended');
  });
  ...
  it.each([
    ...
    ['ended', { kind: 'ended', session: SESSION } as SessionStatus, 'ended'],
