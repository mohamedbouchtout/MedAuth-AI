 ...
 *
 * **A deliberate twin of `apps/mobile/src/session/recovery.ts`.** The two answer
 * identically today and are kept apart for the reason `../session/visitPhase`
 * gives at more length: each is its app's own screen advice, and what keeps them
 * honest is the exhaustive `never` below rather than a shared file. One piece of
 * advice already has to differ in wording — `permission` sends a mobile provider
 * to the OS settings app, and a browser has no such place to send them — so a
 * shared table would have needed a platform parameter on its first use.
 *
