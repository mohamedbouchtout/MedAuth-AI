/**
 * `useNudgeStream` — one encounter's clinical nudges, live on the device.
 *
 ...
 * receive a frame — and a rejected token therefore arrives here as a socket that
 * closes without ever opening, rather than as a close carrying code 4401.
 *
 * **This is `apps/web`'s hook with one deliberate difference.** React Native's
 * `WebSocket` takes a third options argument the DOM's does not, so this app
 * uses the `Authorization` header carrier while the browser has only the
 * subprotocol one. CLAUDE.md's "How the JWT reaches a WebSocket endpoint"
 * explains why both carriers exist and that the choice is a platform constraint
 * rather than a preference. That difference is the whole reason the hook is
 * per-app while the payload parser and the acknowledge client are shared in
 * `@medauth/nudge-client`.
 *
 ...
 * encounter than the audio one does, and `SESSION_TTL_SECONDS` is 15 minutes, so
 * a token close to `exp` is the ordinary case for any visit of real length
 * rather than an edge case. Both halves are implemented — a near-expired token
 * is re-minted before the socket is opened, and a socket that fails to open
 * re-mints once and retries rather than reopening with a token just refused.
 *
 ...
 * this reason. `SessionScreen` calls `startVisit` in one place, and it is not
 * this one.
 *
...
import { isNearExpiry, type ApiFailure } from '@medauth/session-client';
 ...
 * which is how mobile can use the `Authorization` header carrier that `apps/web`
 * cannot. Declared locally, matching `useAudioCapture`, so the call site stays
 * type-checked.
 */
...
export function useNudgeStream({
  ...
  useEffect(() => {
    ...
    const open = (): void => {
      ...
      socket = new (WebSocket as unknown as WebSocketWithHeaders)(
        `${baseUrl}/ws/nudges/${sessionId}`,
        undefined,
      ...
      // There is no `onerror`: React Native fires `error` immediately before
