 ...
 *
 * The app has no `@types/node` and no reason to acquire one — nothing in `src/`
 * runs on Node, and adding the types to build a test fixture would let Node-only
 * globals typecheck inside the app itself. Twenty lines here is the cheaper
 * trade.
 *
 * This is the encoding side of what `src/api/jwt.ts` decodes, kept independent
 * of it so a bug in the decoder cannot be masked by the same bug in the fixture.
 */
